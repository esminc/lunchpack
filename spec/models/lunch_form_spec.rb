...
RSpec.describe LunchForm do
  ...
  describe 'membersのバリデーション' do
    ...
    context '人数は満たしているが、存在しないmembersが指定された場合' do
      let(:members) { %w(鈴木存在しない一郎 鈴木二郎 鈴木三郎) }
    ...
    context '人数を満たしておらず、存在しないmembersが指定された場合' do
      let(:members) { %w(鈴木存在しない一郎 鈴木二郎) }

      it { is_expected.not_to be_valid }

      it 'それぞれのエラーメッセージが返されること' do
        subject.valid?
        expect(subject.errors.messages).to eq(members: %w(3人のメンバーを入力してください 存在する名前を入力してください))
      end
    end
  end
end
